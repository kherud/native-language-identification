...
\end{itemize}
%
Die Netzarchitektur lässt sich dann durch \autoref{eq:embedding} -- \ref{eq:classification} beschreiben (die Batchaxe wird hierbei ausgelassen).
%
\begin{align}
...
Eine zweite LSTM-Schicht zwischen \autoref{eq:zd} und \ref{eq:az}, um potentiell Abhängigkeiten zwischen Zeilen besser zu erfassen, führte empirisch zu keiner Verbesserung. Durch zufällige Suche wurde $e = 100$ und $d = 150$ empirisch ermittelt. \\
...
\end{table}
%
Sowohl Auslassen von Datensätze größerer Klassen als auch Vervielfachen von Dokumente seltener Länder zeigten keine Verbesserung. Erstes verschlechterte die Ergebnisse maßgeblich, letzteres führte hauptsächlich zu einer längeren Trainingsdauer. Stattdessen wurde die Loss-Funktion gewichtet (\autoref{eq:loss-weights}). Der Wert von $\alpha$ wurde dabei durch Ausprobieren aller Werte von 0.0--1.0 in Zehntelschritten ermittelt.
%
\begin{equation}
W_{L,C_i} = \left(\frac{|C_{\max}|}{|C_i|}\right)^\alpha,\;\;\;\alpha = 0.6 \label{eq:loss-weights}
\end{equation}
%
Außerdem wurden verschiedene andere Architekturen ausprobiert.
\begin{itemize}
\item \textbf{CNN}: Hierbei wurden \autoref{eq:lstm}--\ref{eq:zd} durch ein Satz-CNN ersetzt \cite{Kim2014}. Anstatt den Text eindimensional in das CNN zu geben, wurde die zweidimensionale Form $z \times w$ behalten, was die Zeitkomplexität von O($zw$) maßgeblich auf O($w$) reduziert (dabei gehen jedoch potentiell Abhängigkeiten zwischen Zeilen verloren).
